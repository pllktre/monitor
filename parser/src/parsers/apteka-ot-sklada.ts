      ...
      for (const subcat of cat.children || []) {
        if (subcat.children === null) {
          ...
          while (true) {
            ...
            const url = `https://apteka-ot-sklada.ru/api/catalog/search?sort=price_asc&slug=${encodeURI(subcat.slug)}${i < 1 ? "" : "&offset=" + i}&inStock=1&limit=12`;
          ...
          for (const SubSubcat of subcat.children || []) {
            ...
            while (true) {
              ...
              const url = `https://apteka-ot-sklada.ru/api/catalog/search?sort=price_asc&slug=${encodeURI(SubSubcat.slug)}${i < 1 ? "" : "&offset=" + i}&inStock=1&limit=12`;
              const data = await tab.Evaluate(async (url) => {
                const res = await fetch(url);
                const data = await res.json();
