...
const config: InitParserConfig<UniqueParserConfig> = (parser) => ({
  ...
  func: async () => {
    console.log("aptekanabaumana.ru");
    const tab = await create_tab("https://aptekanabaumana.ru/");
    ...
    for (const cat of categories) {
      ...
      await tab.GoTo(`https://aptekanabaumana.ru` + cat.link);
      await sleep(2);
      ...
      for (const subCat of subCategories) {
        ...
        while (true) {
          ...
          const html = await tab.Evaluate(
            async ([url, page]) => {
              const res = await fetch(`https://aptekanabaumana.ru${url}/?perPage=20&page=${page}`);
