...
const config: InitParserConfig<UniqueParserConfig> = (parser) => ({
  ...
  func: async () => {
    ...
    console.log("Current city:", currentCity.replace("и область","").trim());
    ...
    await sleep(10);
    await tab.WaitSelector("ul.container.header__nav-list > li > a");

    const categories = await tab.Evaluate(() => {
      const list = document.querySelectorAll<HTMLLabelElement>(
        "ul.container.header__nav-list > li > a"
      );
      const links = [];
