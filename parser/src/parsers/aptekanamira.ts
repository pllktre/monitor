...
const config: InitParserConfig<UniqueParserConfig> = (parser) => ({
  ...
  func: async () => {
    console.log("aptekanamira");
    const tab = await create_tab("http://aptekanamira.ru/");
    ...
    for (const cat of categories) {
      ...
      await tab.GoTo(`http://aptekanamira.ru` + cat.link);
      await sleep(2);
      ...
      for (const subCat of subCategories) {
        ...
        while (true) {
          ...
          const link = "http://aptekanamira.ru" + href + "/?perPage=20&page=" + page;
          await sleep(4);
          await tab.GoTo(link);
          await sleep(6);
