...
export const create_april_config: (
  o: boolean,
  b: boolean
) => InitParserConfig<UniqueParserConfig> =
  (onOrderOnly, bonus) => (parser) => {
    ...
    return {
      ...
      itemParams: {
        ...
        price: ["div > div.prices > div > span:nth-child(2)"],
        contextID: ["div > a", "href", [/\/product\/(\d*)-/g, 1]],
      },
      itemFunc: async (item, ie) => {
        if (!item.price || !item.name || !brandTab || !item.contextID) return;

        const orderButton = ie.querySelector(
          "div.card-info > div.footer-card > div.button > button > div"
        );

        if (!orderButton) return;

        const isOnOrder = orderButton.textContent.includes("Под заказ");

        const isStock = orderButton.textContent;
        console.log(isStock);

        if (onOrderOnly !== isOnOrder) {
          return;
        }
      ...
      func: async () => {
        ...
        await tab.Click(
          "#app > header > div > section > div.c-catalog > button"
        );
        await sleep(2);
        await tab.Click(
        ...
        for (let i = startCat || 0; i < catCount; i++) {
          ...
          await tab.WaitSelector("#product-list");
...
const config = create_april_config(false, true);
