...
const config: InitParserConfig<UniqueParserConfig> = (parser) => ({
  ...
  func: async () => {
    console.log("aptekanadekabristov");
    const tab = await create_tab("http://aptekanadekabristov.ru/");
    ...
    for (const cat of categories) {
      ...
      await tab.GoTo(`http://aptekanadekabristov.ru` + cat.link);
      await sleep(2);
      ...
      for (const subCat of subCategories) {
        ...
        while (true) {
          ...
          const link = "http://aptekanadekabristov.ru" + href + "/?perPage=20&page=" + page;
          await sleep(4);
          await tab.GoTo(link);
          await sleep(6);
