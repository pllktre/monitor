declare module "nightmare" {
	declare class Nightmare {
	    ...
	    //Ruslan additionals
	    eval<T extends any, R>( func: ( arg: T ) => R, arg?: T ): Promise<R>
	    get_selector_count( selector: string ): Promise<number>
	    get_html( selector?: string ): Promise<string>
	    scroll_to( selector: string ): Nightmare
		wait_click( selector: string, time?: number ): Nightmare
		wait_insert( selector: string, text: string, time?: number ): Nightmare
